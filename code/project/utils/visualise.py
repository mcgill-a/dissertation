...
def plot_attention_weights(encoder_inputs, attention_weights, source_id2word, target_id2word, filename=None):
    ...
    ax.set_xticklabels([target_id2word[inp] if inp !=
                        0 else "<Res>" for inp in dec_inputs])
    ax.set_yticklabels([source_id2word[inp] if inp !=
                        0 else "<Res>" for inp in encoder_inputs.ravel()])
