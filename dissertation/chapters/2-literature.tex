...
% 
Extracting simple features from the lower levels of representation helps to identify the abstract features present in the higher representation levels that are vital to the output classification of data (\cite{bengio_deep_2011}).
%
...
% \cite{kehtarnavaz_comparison_2017} MLP study
...
In general, each convolutional layer involves three different actions (\cite{goodfellow_deep_learning_2016}):
\begin{itemize}
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.5\textwidth]{media/literature/machine_learning/ml_pooling.png}
...
% - Back-propagation (update weights) % REMOVE but mention
% - Pooling layer (sub-sample input after convolutional layers)
% XXX - Activation function (decide if neuron fires or not) % mention briefly
% - Batch normalisation (improve speed + performance, helps reduce overfitting)
...
% why is it different from an MLP?
...
% Pooling Layer
% Activation Function
% Batch normalisation (helps reduce overfitting)
% Dropout
% Fully Connected Layer

% sections from example dissertation:
...
%\subsubsection{Why is it different than a CNN?}
% what is it good for?
...
%\cite{mikolov_recurrent_2010} recurrent neural network slides

%\cite{goodfellow_deep_learning_2016} book on machine learning
