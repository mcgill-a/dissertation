...
\section{Low-Resource Data Augmentation}
\label{sec:2-low_resource_mt}
% Add a paragraph that provides an overview for low resource MT
%Two important areas of research in low-resource \acrshort{NMT} are data augmentation and neural network initialisation. New data can be generated by augmenting existing datasets to increase the number of training samples that can be used for the low-resource language. A number of these data augmentation techniques will be analysed in Section \ref{sec:2-low_resource_mt}. Using the knowledge gained on a previous task to improve the performance on a related task can be achieved through transfer learning and meta learning techniques, where training on high resource languages forms the basis of the initialisation in the neural network of a low-resource language as described in Section \ref{sec:2-transfer_learning}.
