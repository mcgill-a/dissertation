...
Deep learning is a subset of machine learning inspired by the human brain that uses artificial neural networks with many hidden layers to extract features from inputs while training with large data amounts of data. %The deep neural networks that are trained on a large parallel corpus are the 
...
%These pooling methods are visualised below in Figure \ref{fig:cnn_pooling}:
...
Predictions are made based on the vectors that pass through the gates and a copy is kept for the next time step. This means future predictions can be informed by memories that have yet to be forgotten. Referring back to previous time steps allows \acrshort{LSTM}s to better represent language specific grammar structures and transfer the meaning of a sequence to other languages.
...
Typically, sequence to sequence problems are solved by stacking both the encoder and decoder with layers of an RNN with \acrshort{LSTM} (\cite{luong_effective_2015}). However, recent research by \cite{gehring_convolutional_2017} proposes the first fully connected \acrshort{CNN} for sequence to sequence learning that outperforms high performance \acrshort{LSTM} translation models by up to 1.9 \acrshort{BLEU}. This approach is able to discover more sequence compositional structure than \acrshort{RNN}s due to the hierarchical representations of sequences.
...
\hline
\multicolumn{1}{|c|}{\textbf{Language(s)}} & \textbf{Data Type} & \multicolumn{1}{c|}{\textbf{Sentences}} & \multicolumn{1}{c|}{\textbf{Source}} \\ \hline
\begin{tabular}[c]{@{}l@{}}Scottish Gaelic (GD)\\ English (EN)\end{tabular} & Bilingual & 57.5k & OPUS: GNOME v1 (\cite{tiedemann_opus_2012}) \\ \hline
\begin{tabular}[c]{@{}l@{}}Scottish Gaelic (GD)\\ English (EN)\end{tabular} & Bilingual & 36.6k & OPUS: Ubuntu v14.10 (\cite{tiedemann_opus_2012}) \\ \hline
\begin{tabular}[c]{@{}l@{}}Scottish Gaelic (GD)\\ English (EN)\end{tabular} & Bilingual & N/A & LearnGaelic PDF Materials (\cite{learn_gaelic_2019})\\ \hline
%\begin{tabular}[c]{@{}l@{}}Scottish Gaelic (GD)\\ English (EN)\end{tabular} & Bilingual & N/A & Scottish Parliament Publications \\ \hline
